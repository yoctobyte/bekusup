...
class IndexStore:
    ...
    def log_session(self, drive_id: str, session_id: str, outcome: str):
...
def verify_trust(disk_node, mountpoint, index: IndexStore):
    ...
    if marker.get("serial") and serial and marker.get("serial") != serial:
        return False, f"Marker serial '{marker.get('serial')}' conflicts with actual serial '{serial}'."
        
    if marker.get("uuid") and uuid and marker.get("uuid") != uuid:
        return False, f"Marker UUID '{marker.get('uuid')}' conflicts with actual UUID '{uuid}'."
        
    drive_id = serial if serial else f"{uuid}-{label}"
    if drive_id in index.data["drives"]:
